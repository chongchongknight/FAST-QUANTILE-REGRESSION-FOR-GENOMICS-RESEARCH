function [estimate_cell, pvalue_cell, ztime_cell, xiter_cell, piter_cell, totaltime_cell] = qr_add_new4(z,x,y,tau,nchg,varargin)
...
validateattributes(tau, {'numeric'},{'2d'},'qr_add','quantile level tau',4)
[tau_length, ~] = size(tau);
...
estimate_cell = cell(tau_length, 1);
pvalue_cell = cell(tau_length, 1);
ztime_cell = cell(tau_length, 1);
xiter_cell = cell(tau_length, 1);
piter_cell = cell(tau_length, 1);
totaltime_cell = cell(tau_length, 1);

for tt =1:tau_length
    t0 = tic;
    % c in min(c^T X)
    c = [zeros((1+nvar),1); tau(tt)*ones(m,1); (1-tau(tt))*ones(m,1)];
    ...
    if tt == 1
        ...
        tem=[ones(m,1) z eye(m,m) -eye(m,m)];
        bb=y;
        tem(bb<0,:)=-tem(bb<0,:);
        bb(bb<0)=-bb(bb<0);
        ...
        temr = c' - c(IB)' * invAib * tem;
        %temr = gammax(end,:);
        tem_gammax = invAib * tem;
        gammax = [tem_gammax(:,r1); temr(r1)];
        %gammax = [tem_gammax(:,r1); temr];
        b = invAib * bb;
    ...
    while (j<inp.Results.maxit)
        ...
        if r2(t_rr)==0
            ...
            if gammax(end,t_rr)<0
                ...
                % step 5 
                % impossible, if r2(t_rr)==0, then tsep must be 1, gammax(end,t_rr)<0
                k=-b./ga;
                k=find(k==min(k(ga<0 & ~freevarrow )) & (ga<0) ,1);
            end
            % pivoting
        ...
        % in the above step, the trr column of gammax actually store the value 
        % of IB(k), not original 1:nvar+1
        gammax=gammax-ee*gammax(k,:);
        % row transformation 
        b=b-ee(1:end-1)*b(k);
        IB(k)=t; 
        % basic colum k becomes t
    
        %tem1 = eye(m);
        %tem2 = ee(1:end-1,:);
        %tem = tem1 - full(sparse(1:m,k,tem2,m,m));
        %prod_inv = tem*prod_inv;
    ...
    cib=c(IB);
    % IB is basic column id
    invAib=[ones(m,1) z eye(m,m) -eye(m,m)];
    invAib(y<0,:)=-invAib(y<0,:);
    invAib=invAib(:,IB);
    % invAib = inv(invAib);

    bd = band(tau(tt), m, inp.Results.hs);
    h1 = (norminv(tau(tt) + bd) - norminv(tau(tt) - bd));
    ...
    for i = 1:nchg:(nx-nchg+1)
        %t3 = tic;
        [temest, ~] = multiaddmyqr3(x(:,i:(i+nchg-1)),nvar,r1,r2,gammax,...
            BinvX(:,i:(i+nchg-1)),cib,b,IB,freevarrow,y,inp.Results.tol,inp.Results.maxit);
        estimate = [estimate temest];
        %xiter = toc(t3);
    end
    %xiter = toc(t3);
    ...
    estimate_cell(tt) = {estimate};
    pvalue_cell(tt) = {pvalue};
    ztime_cell(tt) = {ztime};
    xiter_cell(tt) = {xiter};
    piter_cell(tt) = {piter};
    totaltime_cell(tt) = {totaltime};
end
end
