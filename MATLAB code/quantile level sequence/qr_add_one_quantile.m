function [estimate, pvalue, ztime, xiter, piter, totaltime] = qr_add(z,x,y,tau,nchg,varargin)
...
%tem_gammax =prod_inv* [ones(m,1) z eye(m,m) -eye(m,m)];
%tem_gammax2 =invAib* tem;
%disp(sum(sum(tem_gammax(:,1:nvar+1) == gammax(1:end-1,:))))
%disp(sum(sum(tem_gammax2(:,r1) == gammax(1:end-1,:))))
%disp(sum(sum(prod_inv ~= invAib))) exactly the same
%disp(sum(tem_gammax2(33,1:nvar+1) ~= gammax(33,:) ))
%disp(tem_gammax2(1:10,1:nvar+1))
%disp(gammax(1:10,:))

%disp(find(freevarrow ~=0))

%aaa = c' - cib' * invAib * tem;
%bbb = gammax(end,:);
%disp(bbb)
%disp(aaa(r1))
